...
var $ = {
  ...
  'path-complete-extname': require('path-complete-extname')
};
...
function findMainFiles(config, component, componentConfigFile) {
  ...
  if ($._.isString(componentConfigFile.main) && componentConfigFile.main !== '') {
    // start by looking for what every component should have: config.main
    filePaths = [componentConfigFile.main];
  } else if ($._.isArray(componentConfigFile.main) && componentConfigFile.main.length > 0) {
    filePaths = componentConfigFile.main;
  } else if (componentConfigFile.hasOwnProperty('files')) {
    // sg custom property, get all files described in previous spec
    for (var key in componentConfigFile.files) {
      if (componentConfigFile.files.hasOwnProperty(key)) {
        filePaths = filePaths.concat(componentConfigFile.files[key]);
      }
    }
